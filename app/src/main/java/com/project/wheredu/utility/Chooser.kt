...
import com.project.wheredu.circle.Circle
import com.project.wheredu.circle.GroupCircle
import com.project.wheredu.circle.OrderCircle
...
class Chooser(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    private fun createCircle(x: Float, y: Float) = when (mode) {
        Mode.SINGLE -> Circle(x, y, 50f * scale)
        Mode.GROUP -> GroupCircle(x, y, 50f * scale)
        Mode.ORDER -> OrderCircle(x, y, 50f * scale)
    }
